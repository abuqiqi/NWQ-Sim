...
## The major function
def adapt_vqe_gcm(hamiltonian_op, pool, reference_ket, 
                use_qugcm       = False,
                file_prefix = None,
                occupied_list   = None,
                adapt_conver    = 'norm',
    ...
    VQE_DIFFS = [] # Muqing: Error from FCI energy for VQE algorihtm
    VQE_ITERS = [] # number of optimization iterations

    times_grad = [] # Muqing: Time for computing gradients
    times_vqe = [] # Muqing: Time for VQE optimization
    times_gcm = [] # Muqing: Time for GCM
    times_gcmeig = [] # Muqing: Time for GCM eigensolver
...
# {{{
    ...
    if no_printout_level < 1:
        ...
        pool.gradient_print_thresh = theta_thresh * 100
    else:
    ...
    # w_dense, v_dense = scipy.linalg.eigh(hamiltonian.todense())
    # true_lowest_ev = w_dense[0]
    ####
    ...
    for n_iter in range(0,adapt_maxiter):
        ...
        for oi in range(pool.n_ops):
            ...
            if abs(gi) > abs(next_deriv):
                next_deriv = gi
                next_index = oi
        tend_grad = timer()# Muqing: timer
        times_grad.append(tend_grad-tstart_grad) # Muqing: time in seconds
        ...
        if converged:
            ...
            print(" *VQE Finished: %20.12f" % trial_model.curr_energy)
            print(" *GCM Finished: %20.12f" % GCM_RESULT[0])
            
            vqe_diff = np.abs(true_lowest_ev-trial_model.curr_energy)
            gcm_diff = np.abs(true_lowest_ev-GCM_RESULT[0])
            print(" *VQE error {:.6e}".format( vqe_diff ))
            ...
            for si in range(len(ansatz_ops)):
                ...
                # opstrs.append(opstring) # Muqing
                print(" %4i %12.8f %s" %(si, parameters[si], opstring) )
            print(" ---------------------- ")
            # print(parameters) # # Muqing
            # for ooo in opstrs:  # Muqing
            #     print("\""+ooo+"\",") # Muqing
            break
        ...
        if not make_orth:
            if not use_qugcm:
                tstart_gcm = timer() # Muqing: timer
                gcmH, gcmS, prodBasis_set = gcm_mats(hamiltonian, ansatz_mat, reference_ket, parameters,
                                                    prev_basis = prodBasis_set, make_orth = make_orth)
                tend_gcm = timer() # Muqing: timer
                times_gcm.append(tend_gcm-tstart_gcm) # Muqing: time in seconds
            else:
                # if occupied_list is None:
                #     raise Exception("Please input occupied list for creating HF state in circuit")
                # num_qubits = int( np.log2(hamiltonian.shape[0]) )
                # qis_occupied_list = [num_qubits-1-oind for oind in occupied_list]
                # gcmH, gcmS, prodBasis_set = qugcm_mats(hamiltonian_op, ansatz_ops, qis_occupied_list, parameters, 
                #                                        num_qubits, PAULI_EXP_TABLE,
                #                                     prev_basis = prodBasis_set)
        ...
        if file_prefix is not None:
            ...
            np.save(file_prefix+'IterMats/Iter{:d}_S.npy'.format(n_iter), gcmS)
            np.save(file_prefix+'IterMats/Iter{:d}_H.npy'.format(n_iter), gcmH)
            # np.save(file_prefix+'IterMats/Iter{:d}_bases.npy'.format(n_iter), np.array(ansatz_mat))
            np.save(file_prefix+'IterMats/Iter{:d}_preop_coeffs.npy'.format(n_iter), np.array(parameters))
        tstart_gcmeig = timer() # Muqing: timer
        if not make_orth:
            GCM_RESULT = gcm_eig(gcmH, gcmS, ev_thresh=ev_thresh, 
                                printout=True)
        else:
            GCM_RESULT = sl.eigh(gcmH)[0]
        tend_gcmeig = timer() # Muqing: timer
        times_gcmeig.append(tend_gcmeig-tstart_gcmeig) # Muqing: time in seconds
        ...
        if file_prefix is not None:
            try:
                ...
                np.save(file_prefix+'Iter{:d}_S.npy'.format(n_iter), iter_time_arr)
    ## save time
    if file_prefix is not None:
    ...
    return trial_model.curr_energy, curr_state, parameters, GCM_DIFFS, VQE_DIFFS, GCM_BASIS_SIZE, VQE_ITERS
