...
from pyscf.tools import molden # Muqing
...
print("This is a modified pyscf_helper.py by Muqing Zheng")
...
def init(molecule,charge,spin,basis,n_frzn_occ=0, n_act=None, mo_order=None, unit='AU'):
# {{{
    #PYSCF inputs
    print(" ---------------------------------------------------------")
    print("                                                          ")
    ...
    mol = gto.Mole()
    mol.atom = molecule
    mol.unit = unit # Muqing: add unit option
    print("Choosing unit", mol.unit)
    print("Symmetry: ", mol.symmetry)
    ...
    mol.spin = spin
    mol.basis = basis
    # mol.frozen = n_frzn_occ # Muqing: add the value
    mol.build()
    ...
    # #SCF 
    mf = scf.RHF(mol).run()
    if not mf.converged: # Muqing: Need convergence check
        print("Warning: SCF did not converge, go to something different")
        mf = scf.RHF(mol)
        mf.max_cycle=200
        mf.init_guess='atom' # one of ‘minao’, ‘atom’, ‘huckel’, ‘hcore’, ‘1e’, ‘chkfile’. Default is ‘minao’
        mf = mf.run()
    # mf = scf.ROHF(mol).run()
    if mo_order != None:
    ...
    # return(n_orb, n_a, n_b, h, eri_act, mol, E_nuc ,mf.e_tot,Cact,S) # Muqing: I think it is n_orb not n_act, both variables has the same value when n_act is initialized None
# }}}
